...
import { appClient } from "@/frontend/lib/api-client";
...
export class ServerMemoService extends MemoService {
  async getMemos(): Promise<Memo[]> {
    try {
      ...
      const response = await appClient.api.memos.$get();
  ...
  async updateMemo(id: string, updates: Partial<Memo>): Promise<Memo> {
    try {
      const response = await appClient.api.memos[':id'].$patch({
        param: { id },
        json: {
  ...
  async createMemo(memo: Omit<Memo, 'id'>): Promise<Memo> {
    try {
      const response = await appClient.api.memos.$post({
        json: {
          content: memo.content,
        },
      });
  ...
  async deleteMemo(id: string): Promise<void> {
    try {
      const response = await appClient.api.memos[':id'].$delete({
        param: { id },
      });
